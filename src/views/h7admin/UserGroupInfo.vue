                ...
                v-card
                    ...
                    h7-data-table(:headers="headers" :items="filterData" :loading="$store.state.global.isLoading" :search-text="searchText" :per-page="10" single-line)
                        template(slot="items" slot-scope="{props, index}")
                            td {{index}}
                            td {{props.item.name}}
                            td {{props.item.user_group_mapping.level == 1 ? 'Admin' : 'User'}}
                            td {{props.item.email}}
                        ...
                        v-window-item(:value="2")
                            ...
                            v-card-text
                                ...
                                v-form(ref="addUserForm")
                                    ...
                                    v-select(v-model="userInfo.level" :items="roleItems" item-text="name" item-value="value" label="Role" :rules="[rules.required]")
                    ...
                    v-card
                        ...
                        v-card-text
                            v-form(ref="editForm")
                                ...
                                v-select(v-model="userInfo.role" :items="roleItems" item-text="name" item-value="name" label="Role" @change="chooseRole(userInfo.role)")
                        v-card-actions
                            v-spacer
                            v-btn(color="grey" flat="flat" @click="closeDialog()") Cancel
                            v-btn(color="primary" flat="flat" @click="updateUserLevel()") Save
            //- remove user dialog
...
export default {
    ...
    methods: {
        chooseRole(role) {
            // console.log(role);
            this.roleItems.forEach((o, i) => {
                if (o.name == role) this.userInfo.level = o.value;
            });
        ...
        editUser(data) {
            ...
            this.userInfo.level = data.user_group_mapping.level;
            this.roleItems.forEach((o, i) => {
                if (o.value == this.userInfo.level) this.userInfo.role = o.name;
            });
        },
        removeUser(data) {
            this.userInfo = Object.assign({}, data);
        ...
        updateUserLevel() {
            // console.log(this.userInfo, "updateLevel");

            this.$store.dispatch("global/startLoading");
            this.$store
                .dispatch("userGroup/updateUserLevel", this.userInfo)
                .then(
                    function(result) {
        ...
        addUserToGroup() {
            ...
            this.$store.dispatch("global/startLoading");
            this.$store
                .dispatch("userGroup/newUserToGroup", this.userInfo)
                .then(
                    function(result) {
                        this.getGroupUserInfo();
                        this.$store.dispatch(
